...
// Check if the user is already registered with email or username
const checkUserRegistration = async (email: string, username: string) => {
  try {
    ...
    return data.some((user: any) => user.email === email || user.username === username);
  } catch (error) {
...
export default function Signup () {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Check if the user is already registered with email or username
    const isAlreadyRegistered = await checkUserRegistration(formData.email, formData.username);
    ...
    try {
      ...
      const user = {
        ...
        confirmPassword: formData.confirmpassword, // Send the plain text password
      };
      console.log(user);
      // Send the POST request to the API
      const response = await axios.post(
        "https://urlpt.technians.in/register/",
        user,
        {
          headers: {
  ...
  return (
    <form onSubmit={handleSubmit}>
        ...
        <label
          htmlFor="Username"
          className="mb-2.5 block font-medium text-dark dark:text-white absolute top-[-11px] left-[15px] bg-white px-3 py-0 z-9 text-sm"
        >
          Username
        </label>
      ...
      <div className="mb-4.5 mt-10">
        <button
          type="submit"
          className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-lg submit_btn p-4 font-medium text-white transition hover:bg-opacity-90 hover:bg-primary submit_btn"
        >
          Submit
        </button>
      </div>
